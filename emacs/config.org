  ...
  :config
    (let* ((config-path (concat (getenv "XDG_CONFIG_HOME") "/.luarc.json"))
         (lua-args (if (file-exists-p config-path)
                     ...
                     '("lua-language-server"))))
    (setf (alist-get 'lua-mode eglot-server-programs nil nil #'equal) lua-args)))
#+end_src
...
(use-package mlir-mode
  :straight nil
  :after (exec-path-from-shell eglot)
  :mode (("\\.mlir\\'" . mlir-mode)
         ("\\.pdll\\'" . mlir-mode))
  :config
  (add-to-list 'eglot-server-programs `(mlir-mode . my-eglot-strict-mlir))
  (add-to-list 'eglot-server-programs `((mlir-mode :language-id "pdll") . my-eglot-strict-pdll)))
#+end_src
*** tablegen
...
(use-package eglot
  ...
  (setf (alist-get '(c-mode c++-mode) eglot-server-programs nil nil #'equal)
        '("clangd"
          "--query-driver=/**/*"
          "--background-index"
          "--clang-tidy"
  ...
  (dolist (server-rule '((latex-mode "texlab")
                         (nix-mode "nil")
                         (python-mode "ty" "server")
                         ((sh-mode bash-ts-mode) "bash-language-server" "start")))
    (let ((modes (car server-rule))
          (args (cdr server-rule)))
      (setf (alist-get modes eglot-server-programs nil nil #'equal) args)))
