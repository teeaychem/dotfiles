...
#+begin_src sh
emacsclient -nw -f ~/.config/emacs/var/server/auth/server
/Applications/Emacs.app/Contents/MacOS/bin/emacsclient -n -c -f ~/.config/emacs/var/server/auth/server
#+end_src

Though if `EMACS_SERVER_FILE` is set, `-f` is only relevant as an override.
TCP is used for remote connexions.
